...
public class Grid extends JPanel{
    ...
    private ArrayList<Integer> eulerTour;
    private SharedMemory shmem;
    public Grid(int h, int w, int newCellSize, SharedMemory shmem){
        super();
        ...
        grid = new Cell[h][w];
        to = new int[h*w][h*w];
        treeDist = new int[h*w][h*w];
        cellPath = null;
        for(int i=0;i<h;i++) for(int j=0;j<w;j++) grid[i][j]=new Cell();
    }
    public boolean canMove(int u, int d){
        int i1=u/w, j1=u%w;
        int i2=i1+Directions.dh[d], j2=j1+Directions.dw[d];
        if(i1<0 || i1>=h || j1<0 || j1>=w) return false;
        if(i2<0 || i2>=h || j2<0 || j2>=w) return false;
        return grid[i1][j1].getWallState(d) == WallState.NONE;
    }
    private void dfs(int u, int p){
        ...
        for(int d=0;d<4;d++){
            if(grid[u/w][u%w].getWallState(d)==WallState.NONE){
                ...
                if(nx>=0 && nx<h && ny>=0 && ny<w && nx*w+ny!=p){
                    dfs(nx*w+ny,u);
                }
            }
        }
    ...
    public void calcOptPaths(){
        for(int start=0;start<h*w;start++){
            boolean[][] visited = new boolean[h][w];
            ArrayDeque<Integer> q = new ArrayDeque<Integer>();
            visited[start/w][start%w] = true;
            q.push(start);
            while(!q.isEmpty()){
                int u=q.poll();
                for(int d=0;d<4;d++){
                    if(grid[u/w][u%w].getWallState(d)==WallState.NONE){
                        ...
                        if(nx>=0 && nx<h && ny>=0 && ny<w && !visited[nx][ny]){
                            visited[nx][ny]=true;
                            to[start][nx*w+ny]=(start==u?nx*w+ny:to[start][u]);
                            treeDist[start][nx*w+ny]=treeDist[start][u]+1;
                            q.add(nx*w+ny);
                        }
                    }
                }
    ...
    protected void paintComponent(Graphics g) {
        ...
        Occupant[][] shmemOccupants;
        synchronized(shmem){
            shmemOccupants=shmem.getOccupants().clone();
        }
        ...
        ctx.fillRect(cellSize*(w/2), cellSize*(h/2), cellSize, cellSize);
        for(int i=0;i<h;i++){
            for(int j=0;j<w;j++){
                if(shmemOccupants[i][j]==Occupant.BUNNY){
                    ctx.setColor(new Color(100,100,255));
                    ctx.fillRect(j*cellSize+cellSize*3/8,i*cellSize+cellSize*3/8, cellSize/4, cellSize/4);
                }
                else if(shmemOccupants[i][j]==Occupant.ROBOT){
